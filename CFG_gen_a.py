...
def ifelse_CFG(lines: list[str], index: int, node: str, Nodeinfo: list[str]):
    ...
    if any(x in ifelse_trig for x in line_lst):
        ...
        curr_node: int = 0
        node1 = node
        node_discard = ""           # To keep the discarded Nested Block
        iter_no = 0
        skip_lines_1 = 0
...
def nestedcasestatement(lines: list[str], index4: int, node: str, Nodeinfo: list[str]):
    ...
    if len(case_trig_act) > 0:
        ...
        while iter_value < len(case_blck[1:]):  # end of the case block will be lines[index4 + iter_value + 1]
            ...
            elif re.search(assgmnt_pttrn, nextline) is not None and 'if' not in re.split(r"() ", nextline):
...
def get_assgnment(line: str, node: str, Nodeinfo: list[str]):
    ...
    assgmnt_pttrn_3 = re.compile(r"\s*(.+)\s*=\s*(.+)\s*(>=)\s*(.+);\s*")
    line_lst = line.split()
    if re.search(assgmnt_pttrn_1, line) is None and re.search(assgmnt_pttrn_2, line) is None and re.search(assgmnt_pttrn_3, line) is None:
    ...
    if not any(x for x in line_lst if x in skip_term_lst):
        if re.search(assgmnt_pttrn_1, line) is not None:
            ...
            assgn_val: str = line_lst[line_lst.index("=") + 1]
            line_s = node + " :: " + assgn_trgt.strip() + " <== " + assgn_val.strip()
        elif re.search(assgmnt_pttrn_3, line) is not None:
            line_lst = [x.strip() for x in list(re.findall(assgmnt_pttrn_3, line)[0]) if x != ""]
            assgn_trgt: str = line_lst[line_lst.index("=") - 1]
            assgn_val: 1
            line_s = node + " :: " + assgn_trgt.strip() + " <== " + assgn_val
...
def for_loop_CFG(lines, line_no, node, Nodeinfo, curr_node, inline_cond_pattern, skip_term_lst, assgnmnt_pattern_o_alwys):
    for_keywrd: str = 'for'
    line0 = lines[line_no]
    line = re.sub(r'[\t\n]', '', line0)
    line_lst = re.split(r'[() ;]', line)

    if line_lst[0] == for_keywrd:  # check if line starts with 'for'
        for_line = ''
        while ';' not in line:
            for_line += ' ' + line
            line_no += 1
        ...
        # for(initia; condition; increment)
        for_parts = [part.strip() for part in re.split(r'[()]', for_line) if part.strip() != ''][1].split(';')
        initial = for_parts[0]
        condition = for_parts[1]
        increment = for_parts[2]

        Nodeinfo.append(node + ' :: ' + initial + '; ' + condition + '; ' + increment + ' :: ' + 'F')

        # to process the body of the for loop
        skip_lines = 0
        while line_no < len(lines) and 'end' not in lines[line_no]:
            line_no += 1
            line = re.sub(r'[\t\n]', '', lines[line_no])
            node_body = node + ', ' + str(curr_node)
            if re.search(inline_cond_pattern, line) is not None and not any(x in skip_term_lst for x in line_lst):
                node_body, Nodeinfo = inline_cond_assgnmnt(line, node_body, Nodeinfo)
                Nodeinfo[-1] = Nodeinfo[-1].replace(' :: C', ', 0 :: C')
                curr_node += 1
            elif re.search(assgnmnt_pattern_o_alwys, line) is not None and not any(x in skip_term_lst for x in line_lst):
                node_body, Nodeinfo = get_assgnment(line, node_body, Nodeinfo)
                Nodeinfo[-1] = Nodeinfo[-1].replace(' :: A', ', 0 :: A')
                curr_node += 1
            # Add additional handling for if, case
            line_no += 1

    line_no += 1
    return node, Nodeinfo, line_no, curr_node
...
def gen_CFG(file_loc: str):
    ...
    while line_no < len(lines):
        ...
        if any(x in always_keywrd for x in line_lst):
            ...
            curr_node += 1
        elif 'for' in line_lst:
            node, Nodeinfo, line_no, curr_node = for_loop_CFG(lines, line_no, node, Nodeinfo, curr_node, inline_cond_pattern, skip_term_lst, assgnmnt_pattern_o_alwys)
...
def main():
    ...
    # ================================================ Folder Location =================================================
    # ==================================================================================================================
    root_loc: str = 'C:/Users/sriva/OneDrive - The University of Texas at Dallas/Rtl_Concolic/RTL_Concolic_Testing_st/RTL_Concolic_Testing-main/'
    # root_loc: str = 'D:/DAC_ext'
    #RTL_loc: str = 'RTL/opentitan-master-PUBLIC/hw/ip/aes/rtl/'
    # root_loc: str = 'D:/DAC_ext'
    RTL_loc: str = 'RTL/'
    ...
    file_lst = ["C:/Users/axs220007/OneDrive - The University of Texas at Dallas/Rtl_Concolic/RTL_Concolic_Testing_st/RTL_Concolic_Testing-main/RTL/trail_compaes.sv"]
